...
class LoginView(APIView):
    permission_classes = [AllowAny]  # Permet l'accès à tout le monde, même aux utilisateurs non authentifiés
    def post(self, request):
        ...
        # Si l'utilisateur est invalide, renvoyer une erreur
        if user is None:
...
class ResetPasswordView(APIView):
    ...
    def post(self, request, uidb64, token):
        ...
        # Obtenir le nouveau mot de passe depuis la requête
...
class ChangePasswordView(APIView):
    ...
    def post(self, request):
        ...
        if not old_password or not new_password:
            return Response({"error": "Les deux champs (ancien mot de passe et nouveau mot de passe) sont requis."}, status=status.HTTP_400_BAD_REQUEST)
